...
import { TokenPairModal } from "./TokenPairModal";
import type { Token } from "@/types/trading";

interface TradingTabsProps {
  selectedPair: { base: string; quote: string };
  onPairChange: (pair: { base: string; quote: string }, token?: Token) => void;
}

export const TradingTabs = ({ selectedPair, onPairChange }: TradingTabsProps) => {
  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleSelectPair = (pair: { base: string; quote: string }, token: Token) => {
    onPairChange(pair, token);
  };
    ...
    <>
      <div className="flex items-center gap-2 bg-card border border-border rounded-lg p-2">
        ...
        <button 
          ...
          className="flex items-center justify-center w-8 h-8 hover:bg-accent rounded-md transition-colors"
        >
